...
function LoginPage() {
  ...
  const [loginRole, setLoginRole] = useState<"merchant" | "customer" | null>(() => {
    if (typeof window !== "undefined") {
      return sessionStorage.getItem("loginRole") as any;
    }
    return null;
  });

  // Auto-resume: if Privy session is already persisted, finish login based on role.
  useEffect(() => {
    if (!privy.ready || !privy.user || sessionUser) return;
    
    const finishLogin = async () => {
      // If we know the role or can detect it
      if (loginRole === "customer") {
        const u = loginAsCustomerWallet(privy.user.walletAddress);
        if (!u) {
    ...
    finishLogin();
  }, [privy.ready, privy.user, sessionUser, loginMerchantWithPrivy, loginAsCustomerWallet, navigate, loginRole]);
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="space-y-2 pt-2">
              <p className="text-center text-[11px] text-muted-foreground">
                Secure login via Privy with your favorite Wallet (MetaMask, Rabby, etc.)
              </p>
        ...
        <Card className="p-6 shadow-elegant">
          ...
          <div className="mt-6 space-y-4">
            <p className="text-[11px] text-muted-foreground">
              Sign in with the wallet address your merchant added for you to view your dues and make payments.
            </p>
            <Button onClick={handleCustomerLogin} variant="outline" className="w-full py-6 text-base font-semibold">
