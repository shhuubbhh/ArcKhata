...
interface State {
  ...
  loginMerchantWithPrivy: (profile: PrivyProfile) => Promise<{ user: SessionUser; merchant: Merchant; created: boolean }>;
    ...
    (set, get) => ({
      ...
      loginMerchantWithPrivy: async (profile) => {
        try {
          ...
          // Only update if user changed to avoid redundant re-renders
